...
module.exports = app => {
    ...
    const user = require("../controllers/user.controller");//full contoller shital ma import kravyu

    const authJwt = require("../middleware/authJwt");
    var router = require("express").Router();//router ma express.js import kravyu

    router.post("/signup", user.validate("signup"), user.signup);//user.create ma je create(method) che a controller ma thi lai aavya
    router.post("/signin", user.signin);//user.create ma je create(method) che a controller ma thi lai aavya
